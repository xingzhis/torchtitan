...
def pick_dtype(dtype_arg: str) -> str:
    if dtype_arg and dtype_arg != "auto":
        return dtype_arg
    # vLLM defaults to auto which typically uses float16 on GPU
    return "auto"


def build_model_args(args, dtype: str, tensor_parallel_size: int, model_id: str) -> str:
    """Build model_args string for vLLM backend."""
    parts = [
    ...
    # Add GPU memory utilization (default 0.9, can be tuned)
    if args.gpu_memory_utilization:
        parts.append(f"gpu_memory_utilization={args.gpu_memory_utilization}")

    # Add max_model_len if specified (can help with OOM issues)
    ...
    # Data parallel size for multi-node setups
    if args.data_parallel_size > 1:
        parts.append(f"data_parallel_size={args.data_parallel_size}")
...
def run_eval(args, model_id: str, output_dir: str, log_path: str):
    ...
    log("=== LMEval (vLLM) ===", filepath=log_path)
    log(f"Model: {model_id}", filepath=log_path)
    log(f"Dtype: {dtype} | Tensor Parallel Size: {tensor_parallel_size}", filepath=log_path)
    if args.data_parallel_size > 1:
        log(f"Data Parallel Size: {args.data_parallel_size}", filepath=log_path)
    if args.gpu_memory_utilization:
        log(f"GPU Memory Utilization: {args.gpu_memory_utilization}", filepath=log_path)
    if args.max_model_len:
        log(f"Max Model Length: {args.max_model_len}", filepath=log_path)
    ...
    # Create vLLM model once and reuse it for both evaluations
    log("Loading vLLM model...", filepath=log_path)
    lm = VLLM(
        pretrained=model_id,
        dtype=dtype,
        tensor_parallel_size=tensor_parallel_size,
        trust_remote_code=True,
        gpu_memory_utilization=args.gpu_memory_utilization if args.gpu_memory_utilization else 0.9,
        max_model_len=args.max_model_len,
        seed=args.seed,
        data_parallel_size=args.data_parallel_size if args.data_parallel_size > 1 else 1,
        batch_size=args.batch_size,
    )
    log("Model loaded successfully!", filepath=log_path)

    # Run evaluations - reuse the same model instance
...
def main():
    ap = argparse.ArgumentParser(description="Run lm-eval on a model using vLLM backend.")
    ...
    if args.output_dir is None:
        args.output_dir = os.path.abspath(f'./results/pretrain_{model_str}_{"-".join(args.dataset_name.split("/"))}_lr-{args.lr}_token-{args.train_tokens}_disp-{args.dispersion}-{args.dispersion_coeff}-{args.dispersion_loc}-tau_cos-{args.tau_cos}-tau_l2-{args.tau_l2}_fewshot-{args.num_fewshot}_maxsample-{args.max_eval_samples}_seed-{args.seed}_vllm')
